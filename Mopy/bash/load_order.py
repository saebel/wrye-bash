...
def _updateCache(lord=None, actiSorted=None, __redo=False):
    ...
    try:
        ...
        if cached_lord is not __empty and (
                _current_deque_index < 0 or cached_lord != _saved_load_orders[
            _current_deque_index]):
            global _current_deque_index
            if not __redo: # either getting or setting
                del _saved_load_orders[_current_deque_index + 1:]
                _saved_load_orders.append(cached_lord)
                _current_deque_index += 1
            else:
                 _saved_load_orders[_current_deque_index] = cached_lord
...
def undo_load_order():
    global _current_deque_index
    if _current_deque_index <= 0: return cached_lord
    _current_deque_index -= 1
    return __redo()
