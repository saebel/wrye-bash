...
class Installer_Wizard(OneItemLink, _InstallerLink):
    ...
    def _apply_tweaks(self, installer, ret, ui_refresh):
        ...
        for iniFile in ret.IniEdits:
            ...
            if iniFile in installer.data_sizeCrc or any(
                ...
                if not ret.Install and not any(
                    ...
                    #  actually installed.  Since
                    # BAIN is setup to not auto install after the wizard,
                    # we'll show a message telling the
        ...
        if lastApplied is not None and BashFrame.iniList is not None:
            ...
            BashFrame.iniList.panel.tweakContents.RefreshTweakLineCtrl(
                lastApplied)
